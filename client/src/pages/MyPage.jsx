...
function MyPage({ currentUser, setCurrentUser }) {
  ...
  const fetchSessions = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/sessions/user/${currentUser._id}`);
      const data = await response.json();
  ...
  // Remove course from current user
    ...
    try {
        ...
        // Update current user without reloading the page
    ...
    <section style={styles.pageSection}>
      ...
      <div style={styles.card}>
        ...
        {currentUser.courses && currentUser.courses.length > 0 ? (
          ...
          <p style={styles.message}>No courses added to your profile yet.</p>
        )}
      </div>
      ...
      <div style={styles.card}>
        <div style={styles.sectionHeader}>
          ...
          <button onClick={() => setShowForm(!showForm)} style={styles.addSessionButton}>
            <Plus size={16} />
            <span>{showForm ? "Close form" : "Add study session"}</span>
          </button>
        </div>
        ...
        {loadingSessions && <p style={styles.message}>Loading study sessions...</p>}
        {errorSessions && <p style={styles.error}>{errorSessions}</p>}
          ...
          <table style={styles.table}>
            ...
            <tbody>
              {sessions.map((session) => (
                <tr key={session._id}>
                  <td style={styles.td}>{session.course?.name || "No course"}</td>
                  <td style={styles.td}>
                    {new Date(session.date).toLocaleDateString("en-CA")}
                  </td>
                  <td style={styles.td}>{session.durationMinutes}</td>
                  <td style={styles.td}>{session.focusLevel}</td>
                  <td style={styles.td}>{session.studyMethod}</td>
                  <td style={styles.td}>{session.notes || "-"}</td>
                  <td style={styles.td}>
                    <button
